...
const Login = () => {
  ...
  const validatePassword = (value) => {
    ...
    else if (!/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%&_])[A-Za-z\d!@#$%&_]{8,16}$/i.test(value)) error = 'userPassword must be between 8 to 16, contains at least  uppercase, lowercase, number and special character (! @ # $ % & _)'
    return error;
  }
  ...
  return <>
    <div>
      ...
      <div style={{ 'display': 'none' }} className='errMsg alert alert-danger w-50 mx-auto text-center'>Incorrect email or password</div>
      <div style={{ 'display': 'none' }} className='sucMsg alert alert-success w-50 mx-auto text-center'>Successfull login, Welcome Back</div>
      <Formik
            ...
            <div className={`container`}>
              <div className={`row`}>
                <div className="col-md-12 py-4 px-0 mx-auto">
                  <Field id='email' validate={validateEmail} className={`${style.inputField} form-control mt-1 w-100`} placeholder='Email' name="userEmail" />
